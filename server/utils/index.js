        ...
        switch (fieldType) {
            ...
            case 'email':
                if (!validator.isEmail(fieldValue)) {
                    throw new ServerError(`Camput ${fieldName} trebuie sa fie email`)
                }
                break;
            case 'boolean':
                if (!validator.isBoolean(fieldValue)) {
                    throw new ServerError(`Campul ${fieldName} trebuie sa fie valoarea booleana`)
                }
                break;
            case 'dateURI':
                if (!validator.isDataURI(fieldValue)) {
                    throw new ServerError(`Campul ${fieldName} trebuie sa fie Date in format uri`)
                }
                break;
